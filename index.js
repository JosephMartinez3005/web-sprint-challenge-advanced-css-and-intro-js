...
function get20s(array){
  ...
  for(let i = 0; i < array.length; i++){
    let dob = array[i].years.slice(0,4);
    let year = eval(dob);
    let dod = array[i].years.slice(6,11)
    let dodyear = eval(dod)
    if(year > 1900 && dodyear < 2000){
...
function addArtist(array){
   array.push({id: 20,
    ...
    bio: "From Oxnard Ca, Turned web designer at the age of 27 years old. Looks forward to contributing to the world in a big way!"}
    )
    return array;
}
